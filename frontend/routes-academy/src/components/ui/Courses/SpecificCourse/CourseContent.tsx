...
const CourseContentSection: React.FC<{ course: Course }> = ({ course }) => {
  const [openIndex, setOpenIndex] = useState<number | null>(null);
  const contentList = course.content_list?.filter(Boolean) ?? [];
  ...
  return (
    <div
      className="bg-[#F8FAFF] rounded-2xl p-3 py-5  md:p-8 scroll-mt-30 "
      id="modules"
    >
      <h2 className="text-3xl font-poppins font-semibold text-[#172554] mb-6">
        Course Content
      </h2>
      <div className="space-y-4">
        {contentList.length > 0 ? (
          <ul className="text-gray-600 leading-relaxed text-sm sm:text-base space-y-3">
          ...
          course.curriculum_detailed.map((item, i) => (
            <div
              key={i}
            ...
            >
              <button
                ...
                style={{
                  backgroundColor: openIndex === i ? "#eef2ff" : "white",
                }}
              >
              ...
              >
                <div className="p-4 pt-0 sm:p-6 sm:pt-0 bg-white border-t border-gray-100">
                  <ul className="text-gray-600 mb-4 leading-relaxed text-sm sm:text-base space-y-2">
                    {item.details.split("\n").map((line, idx) => (
                      <li key={idx} className="flex items-start">
